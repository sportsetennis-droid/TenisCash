...
const card=document.createElement('div');card.style.cssText='background:#fff;color:#171717;border-radius:16px;padding:18px;max-width:480px;width:100%;max-height:85dvh;overflow:auto;font:15px system-ui;box-sizing:border-box';
const text=document.createElement('div');text.style.cssText='white-space:pre-line;line-height:1.35';text.textContent=message;card.appendChild(text);
const actions=document.createElement('div');actions.style.cssText='display:flex;gap:8px;margin-top:14px;position:sticky;bottom:0;background:white;padding:6px 0';
function finish(value){if(!cover.isConnected)return;cover.remove();document.removeEventListener('keydown',key);closeActive=null;resolve(value);}
