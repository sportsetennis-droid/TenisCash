...
async function main() {
  const cases = [
    [229.89, 183.91, 172.42, 36.78, 'SOLO'],
    [249.90, 199.92, 187.43, 39.98, 'STATION 3'],
    [259.90, 207.92, 194.93, 41.58, 'NEW YORK'],
    [279.99, 223.99, 209.99, 44.80, 'RING 4'],
    [299.99, 239.99, 224.99, 48.00, 'CLIMBER RUN'],
    [300.01, 240.01, 225.01, 48.00, 'FORCEKNIT LOW'],
    [399.99, 319.99, 299.99, 64.00, 'CLIMBER PRO 3'],
    [499.99, 399.99, 374.99, 80.00, 'CLIMBER ULTRA'],
  ];
  for (const [base, card, pix, installment] of cases) {
    const offer = calculateOffer(base);
    assert.equal(offer.cardPrice, card);
    assert.equal(offer.pixPrice, pix);
    assert.equal(offer.installmentPrice, installment);
    assert.equal(Math.round(installment * 100) * 4 + Math.round(offer.lastInstallmentPrice * 100), Math.round(card * 100));
  }
  ...
  assert.equal(productOffer(rows[0]).cardPrice, 183.91);
