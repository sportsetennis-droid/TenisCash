...
- ⛔ **REFERÊNCIA = IDENTIDADE DO PRODUTO — ref DIFERENTE = CARD DIFERENTE, SEMPRE** (mesmo com modelo/cor/descrição byte-iguais; igual Kappa K100/K101/K102, mas vale pra **TODA marca**). **NUNCA juntar/consolidar refs diferentes por descrição/cor.** A ref = `cProd` SEM o tamanho (marca com tamanho no cProd) OU o `cProd` INTEIRO (marca com tamanho na DESCRIÇÃO — ex Body for Sure/FILO: `03404` e `04729` da mesma "REGATA PRETO 077" são **2 PRODUTOS = 2 cards**, não 1). O conserto de card com 2+ refs = **SPLIT por ref** (1 card por cProd-modelo), NUNCA merge. ❌ Erro 2026-06-10: consolidei Body for Sure por descrição (merge de refs diferentes); dono: *"não são o mesmo produto, pq tem referencia diferente... vai ter que passar um pente fino em todo o sistema."* Isto **CORRIGE/SUPERA** o "consolidação por DESCRIÇÃO" — descrição/cor igual NÃO junta se a ref difere. **PORQUÊ (dono 2026-06-10, definitivo): a DESCRIÇÃO é GENÉRICA e NÃO define produto.** Código diferente = a peça tem **DETALHES que a diferenciam** (corte/tecido/gola/estampa/acabamento) — *"está descrita da mesma forma, mas a partir do momento que possui um código diferente é porque ela possui detalhes na peça que diferenciam uma regata preta da outra regata preta."* Logo: **agrupar/consolidar por DESCRIÇÃO é PROIBIDO** (o `fix-brand-desc-consolidate.js` de 2026-06-07 está ERRADO na raiz — nunca rodar). 1 card = 1 código, ponto. ⚠️ **2 ARMADILHAS na EXECUÇÃO do split (me explodiram 2026-06-10):** (1) **NUNCA rodar split-por-código numa marca JÁ reestruturada** (color-split/rebuild) — mover `ProductSize` NÃO move `XmlFiscalItem.productId`, então os itens-por-produto ficam DEFASADOS (o card original ainda "vê" todas as cores) → o split RE-conta cores já separadas e DUPLICA (Skechers: 481→626, 74 EANs dup). (2) **NUNCA recalcular `stock`/comprado por barcode incluindo `"SEM GTIN"`** — esse "barcode" existe em milhares de itens de TODA marca; `Σ qCom["SEM GTIN"]`≈79.000 e EXPLODE o comprado (Skechers foi a 79.667). SEMPRE excluir `"SEM GTIN"`. ✅ A trava do split (separa só se um tamanho real aparece em 2+ códigos) funcionou (1207 falso-positivos→34 reais); Body for Sure (testada), Actvitta, N1, Kolosh: OK. Skechers precisa REBUILD da NFe.
  **📋 PENTE FINO 1-card-1-código CONCLUÍDO 2026-06-10 (marca por marca, parser próprio de cada uma):** split aplicado em Body for Sure(19), Skechers(rebuild 41 cards=NFe 481un), Actvitta(1), N1(1), Kolosh(1), **Kappa**(1: calção adulto KP3019012001 × juvenil J001 — tam juvenil 10-16 nos 2 díg finais dos KP…J), **Army**(13 cards por COR literal da descrição — cProd é por-SKU), **Realtex**(1: joelheira 071001×082501), **Vistho**(reorg: letra F/M/G no MEIO do cProd = TAMANHO/faixa 33-38/39-43, `.NNN` = cor; cards eram por-tamanho misturando cores → 1 card/código + 3 splits), **Botafogo**(1: FEM HOME EKBP21196005 × FEM AWAY 21197004 — fornecedor usou o MESMO EAN pra tudo → barcode=cProd; refs curtas 119963-66 = aliases POR-TAMANHO do EKPB, NÃO separa), **Reebok**(3 acessórios multi-cor: mochila Element 5 cores + bolsas Colbie/Plyo — MAS bola 5/7, luva 09-11, caneleira XS/S = TAMANHOS de acessório, não separa), **Caju**(1: short 022.016×020.086). **ZERO merge real (verificado):** Penalty(cProd=código+tam2díg), Diadora(cProd=EAN, ref no nome), Progne(`106229 M`), Hope(HF…+letra), Alto Giro(por-SKU, chave=desc), Asics(`modelo cor4+tam2`), Puma(`modelo cor3+PEQ/MED/GRD/EGR`=tamanhos!), Adidas(ref no nome), Hoka, Salomon, Nike(refs por-tamanho, 1 EAN/ref). **PARKED (não chutar):** boné Reebok CL0082NUBN73U/66U (73/66 = cor OU fitted 7⅜/6⅝ — dono decide) · Lupo (cor/tam só nas CAIXAS; site cobre 9/43) · Nike (NFe NÃO traz cor — colorway misturado indetectável) · fila-curta: Hidrolight, Vollo, cauda de marcas pequenas. **ENCERRAMENTO 2026-06-10 (2ª varredura, pós-realinhamento de 1.308 XmlFiscalItem):** sweep-2refs-pos-realinha.js (a ferramenta de auditoria — roda DEPOIS de qualquer mexida) flagou 134 → conferidos 1 a 1: REAIS consertados = bermuda BFS (entradas desalinhadas), FIBER protetor BLACK×GRAY (meu rebuild tinha tratado cor como tamanho), Vistho VS59 (cards por TAMANHO misturando cores → 3 cards por COR c/ tam 33/38+39/43), N1 joelheira FLYER II×VOLEI BRASIL. FALSOS confirmados (codificação de tamanho do fornecedor — NÃO separar): Salomon/N1/Munich/Actvitta/Brooks/Hidrolight/DRB/Vollo = 1 código POR TAMANHO (1 EAN/ref; conferir por DESC-sem-tamanho); Brooks meio-tamanho com VÍRGULA ("41,5"/"42,"); Caju "C"+P1 = infixo do tamanho P1 (001.018C00042P1 = mesmo produto do 001.01800042M) e chinelo NN-NN (35-36); FILA/UMBRO escrevem a MESMA ref com e sem o tamanho colado (13237 = 1323739-menos-39 → mesmo produto).
...
- **ANTES de commitar arquivo backend (`src/`): conferir que TODOS os `require` resolvem NO REPOSITÓRIO** (`git ls-files <arquivo>` / `git check-ignore`), não só no PC. Arquivo gitignored existe local mas NÃO no build do Railway → `MODULE_NOT_FOUND` → app cai no startup (**502, prod fora do ar**). Push força rebuild e expõe esse tipo de bomba-relógio. **Erro 2026-06-07:** `suppliers.js` importava `../services/stockSync` (gitignored, regra de não-deploy do recalc) → meu push rebuildou → prod 502. Corrigido removendo o require + a chamada `recalcSizeStock` (proibida). Depois de QUALQUER push que mexe em `src/`, **curl no site e confirmar HTTP 200**. **AUTOMATIZADO:** `node scripts/check-requires.js` (rode local ANTES de pushar — confere que todo require relativo do `src/` aponta pra arquivo versionado no git) + CI `.github/workflows/ci.yml` (roda a cada push: npm ci + prisma generate + node --check + check-requires → ✅/❌ na hora).
- **Railway healthcheck = rede de segurança contra deploy quebrado (config 2026-06-07).** No `railway.toml` o healthcheck VAI dentro de `[deploy]` como `healthcheckPath = "/api/health"` (+ `healthcheckTimeout`). ❌ NÃO usar a seção `[healthchecks]` com `path` — o Railway **IGNORA** (foi por isso que o 502 de hoje derrubou o prod: o healthcheck existia mas no formato errado, nunca rodou). Com o `healthcheckPath` certo: deploy que não sobe (crash no boot) é marcado **falho** e o Railway **mantém a versão antiga no ar** → deploy quebrado não derruba mais o prod.
...
- **REGRA INQUEBRÁVEL — Nuvemshop FOTO + DESCRIÇÃO (dono 2026-07-04):** a vitrine SÓ recebe produto com **FOTO** (`imageUrl` OU pelo menos 1 em `imageUrls[]`, string não-vazia) **E DESCRIÇÃO** (`longDescription` OU `shortDescription` não-vazia). Faltando **qualquer um dos dois**, NÃO sobe (nem cria, nem atualiza) — mesma disciplina das 4 classificações. Enforced em `_pushProductToNuvemshopInner` (trava por produto, cobre cron+admin+script) + filtro de volume no `where` de `pushAllProducts`. ⚠️ Bloqueia UPDATE também → produto já publicado sem foto/desc CONGELA (não sincroniza estoque) até ser **despublicado**. Impacto no catálogo (2026-07-04): 2.123/4.341 ativos incompletos; **1.066 já publicados violando** (despublicar) e 1.057 dos sem-mapping seriam barrados.
- **REGRA INQUEBRÁVEL — Nuvemshop ESTOQUE (dono 2026-06-08):** a loja respeita o **ESTOQUE FÍSICO REAL** = `Σ StoreStock` de **TODAS as lojas** (o LOCALIZADO/bipado), **NÃO o comprado** e **NÃO vinculado a loja nenhuma** — "não é pq o produto foi pro site que ele vai pro estoque da LOJA04; estoque é estoque, tem que ser respeitado". E **só sincroniza tamanho com estoque > 0** — pra NUNCA vender um número que não tem. Tamanho que zerou → **esgotado** (stock 0) na loja; produto sem nenhum tamanho disponível → **não publica** (`published=false`). Enforced em `pushProductToNuvemshop` + `buildNuvemshopProductPayload`: usa `Σ StoreStock` como stock da variante, `createSizes` (só disponíveis) no create, `updateSizes` (todos, zerados=esgotado) no update, e `published` exige ≥1 tamanho com estoque. ❌ NUNCA voltar a mandar `ProductSize.stock` (comprado) como estoque da loja.
- **REGRA INQUEBRÁVEL — Markup / custo / preço de venda (dono 2026-06-07):**
  ...
  2. **Produto que vai pra LOJA (storefront/Nuvemshop) SÓ pode ir com o PREÇO DE VENDA COM MARKUP — NUNCA com o custo.** Se `price` estiver igual ao `costPrice` (markup não aplicado) ou `costPrice` vazio, NÃO sobe pra loja. O custo é interno; a loja só vê o preço de venda. (Exceção observada: card com preço de varejo real mas `costPrice=0` por custo não-gravado — pode subir; o preço é verificável contra o "gêmeo" do mesmo modelo que tem custo. O bloqueio real é `price=0` ou `price<=custo`.)

## ⚠️ REGRA PERMANENTE — Nuvemshop: o push dispara webhook `product/created` que DUPLICA o card (ghost) — 2026-06-08

Ao subir produto pro Nuvemshop (`pushProductToNuvemshop`), o Nuvemshop responde com o webhook `product/created`. O handler `upsertLocalProduct` (`src/services/nuvemshopHandlers.js`) busca o mapping por `nuvemshopProductId`; numa **CORRIDA**, o webhook chega ANTES do push gravar o mapping → cai no `else` → o dedup por SKU usa `Product.sku` (referência) mas o `sku` da variante é o **código de barras** → não casa → **CRIA um Product novo cru** (`source='nuvemshop'`, 1 tamanho, sem classificação) + um 2º mapping pro MESMO NS#. Resultado: **1 card-fantasma local por produto subido** (a loja Nuvemshop fica CERTA — variantes/fotos/publicado ok; a bagunça é só local). Foi o que aconteceu ao subir HOKA (11 ghosts).

- **Detectar:** cards ativos com nome duplicado da marca; OU `source='nuvemshop'` recém-criado com 1 size/sem classificação; OU NS# (`nuvemshopProductId`) com >1 mapping. Inspeção = agrupar mappings por `nuvemshopProductId`.
- **Subir marca:** `scripts/push-hoka-nuvemshop.js` (chama `pushProductToNuvemshop`; só ativos + 4 classificações + `price>0` e `>custo`; idempotente, atualiza se já mapeado). 11 tênis HOKA subidos 2026-06-08 (Rincon 4 Fem Verde segurado: preço 0).
- **Limpar ghost (seguro):** `scripts/fix-hoka-ghost-dups.js [--brand=X] [--apply]` — pra cada NS# com fantasma(`source=nuvemshop`)+curado, apaga o fantasma (variant-mapping→product-mapping→storeStock→sizes→product; fallback = desativa) e MANTÉM o curado mapeado. GUARDA: só apaga fantasma que tenha irmão curado no mesmo NS#. Backup + dry-run. HOKA: 11 ghosts removidos, 12 ativos limpos.
- **Raiz CORRIGIDA 2026-06-08 (deployed):** no TOPO de `upsertLocalProduct`, GUARDA ANTI-ECO — se alguma variante casa por `barcode`/`sku` contra `ProductSize.barcode` existente, o produto nasceu no TenisCash (o webhook é só o eco do push) → NÃO espelha (não cria card-fantasma nem sobrescreve curado), só faz `upsert` do mapping no card existente e retorna. E o `create` do mapping no push virou `upsert` (idempotente p/ a corrida). Próximos pushes não criam mais ghost.
- **⚠️ "Converse: 30 mappings, 2 ativos" NÃO é ghost (hipótese errada minha):** os 30 cards Converse são todos `nfe-import` (1 card por NS#, zero fantasma). O real: 28 foram DESATIVADOS local (consolidação) mas seguem **PUBLICADOS na loja Nuvemshop** (28/28 LIVE). Decisão do dono pendente: são duplicatas (despublicar na loja) ou colorways legítimos desativados por engano (reativar local)? — `scripts/fix-hoka-ghost-dups.js --brand=Converse` acha 0 ghost (correto, não serve aqui).
...
- ✅ **REGRA NO BIPE (dono 2026-06-10/11, implementado em prod):** ao bipar produto **ADIDAS** com tamanho **não-confirmado por caixa** (`ProductSize.sizeConfirmedAt` null — cobre placeholder `T-<EAN>` E chute antigo de importação), a tela `/bipar.html` **PEDE o tamanho manual** (bottom-sheet: grade 34–45 + campo livre + pular; sobre chute mostra "No sistema está: X (não confirmado)"). O vendedor lê o número na CAIXA física = a fonte real. Backend: `POST /api/stocktake/bipe` responde `needsSize:true`; `POST /api/stocktake/bipe/:id/size` grava + `sizeConfirmedAt=now()` (valida 15–50/.5/PP–XGG/2A–16A). **CAIXA VENCE CHUTE** (dono: *"eu nao tenho autonomia pra escolher o tamanho"* ao levar 409 de um chute): se o tamanho digitado colide com outro código do card cujo tamanho NUNCA foi confirmado → REMANEJA em transação (chute vira placeholder, pede quando for bipado; o código com a caixa na mão fica com o número). Colisão com tamanho JÁ confirmado por caixa → 409 (conflito real, conferir). Confirmado NUNCA é sobrescrito nem remanejado. 1 código = 1 tamanho: confirmou uma vez, próximos bipes vêm certos. Fallback NFe do bipe cria ProductSize Adidas direto como `T-<EAN>`. **SWEEP TOTAL rodado 2026-06-11** (ordem do dono: "tirar a vinculação do registro atual do código junto ao tamanho... de todos os produtos da adidas"): `scripts/sweep-adidas-unlink-sizes.js --apply` desvinculou os 289 não-confirmados restantes (230 números chutados + 48 Único + 11 lixo) → **TODO código Adidas (1.257) está placeholder e o tamanho SÓ entra manualmente** pelo bipe+caixa. Acessório sem tamanho: botão "Único" no modal (validação aceita U/UNICO/ÚNICO). **Grade do modal = a FAIXA escrita no ITEM** (dono: "no item do produto diz as possibilidades de tamanho"): `extractSizeOptions` lê "34/39"→34..39, "7A/14A"→idades, "P/GG"→P,M,G,GG, "18/24"→infantil, do nome do card (fallback: descrição NFe do código); `POST /bipe` devolve `sizeOptions`; sem faixa detectável → grade padrão 34–45 + botão Único.
...
## REGRA PERMANENTE — Sistema fiscal (agentes, TROCA, cancelamento) — 2026-06-11

**Runbook completo: `docs/fiscal-troca-runbook.md`** (arquitetura, mapa das 6 lojas, procedimentos, armadilhas). Resumo do que NUNCA esquecer:

- **Cada loja emite pela PRÓPRIA máquina** (agente v2.1-troca em `C:\TenisCashAgent`, stateless — issuer vai no body; cert do grupo assina qualquer filial). Matriz FORA da rota fiscal (ordem do dono: "cada máquina precisa estar tudo sobre cada loja"). Failover automático loja-cobre-loja no `fiscalAgentClient`.
- **TROCA nunca é "cupom da diferença"**: devolução NFe 55 (série 2, finNFe=4, CFOP 1202, NFref) + cupom novo VALOR CHEIO com pagamentos `05 Crédito Loja` + diferença. Estoque mexe SÓ no StoreStock (devolvido volta, novo sai); comprado intocado.
- **PB é SVRS no modelo 55 também** — `nfe.sefaz.pb.gov.br` NÃO EXISTE em DNS. Agente 2.0 quebra em mod-55; roteamento por versão manda payload de troca/55 só pra agente >=2.1.
- **NFe própria = SÉRIE 2 em todos os CNPJs** (Chianca usa série 1). Número fiscal autorizado NÃO se reusa nem cancelado (539) — testes sempre com 9000xx novo, R$ baixo, e CANCELAR depois.
- **Ferramentas**: `scripts/fiscal-status.js` (raio-x read-only) · `scripts/fiscal-test-emit.js LOJA0X 9000NN` (teste real c/ cancelamento) · `scripts/fiscal-backup-config.js` (backup com SEGREDOS pra C:\Users\sport\TenisCashBackups — NUNCA commitar).
- Matar agente local: pela PORTA (`Get-NetTCPConnection -LocalPort 8765`) — CommandLine não carrega o path e você mata o processo errado/nenhum.

## Atualização do dono —15/09/2026: conversão de tamanhos pela tabela oficial
Quando a etiqueta mostrar apenas USA/US,UK ouEUR, consultar a tabela oficial da marca e do público/modelo. Equivalência BR inequívoca e documentada pode ser usada para resolver o bipe autorizado, sem exigir BR literal na foto. Guardar tamanho original,BR,URL e data. Esta atualização expressa supera a proibição anterior de conversão documentada; continua proibido chutar, generalizar entre marcas ou usar escala masculina para feminina. Constituição global: C:/Users/dougl/.codex/AGENTS.md, regra11.

## Regra permanente — 15/09/2026: aprendizado de códigos na bipagem
Aplicar a regra 12 da Constituição global (C:/Users/dougl/.codex/AGENTS.md) a todas as marcas/produtos: código não encontrado exige busca pelas referências legíveis e pelas importações NF-e. Identidade e variante comprovadas permitem resolver o bipe e salvar o código efetivamente lido na variante correta, preservando códigos válidos, evidências, empresa, loja e rodada. Nunca inferir vínculo por descrição genérica, gravar SEM GTIN como barcode, sobrescrever vínculo conflitante ou alterar comprado/XML original. Reutilizar os mecanismos existentes de aliases e catálogo canônico; verificar o reconhecimento depois da gravação. Regra documental não é prova de implementação em produção.
...
## 15. Resolver tamanho imediatamente ou em paralelo — 15/09/2026
Ordem atual do Douglas: tamanho pendente não deve paralisar o trabalho. Ao ler o código, consultar imediatamente cadastro e NF-e de entrada em paralelo ao OCR local, sem API paga. EAN exato, vínculo fiscal com produto canônico, referência/cor compatíveis e tamanho literal inequívoco permitem completar ou cadastrar a variante faltante no produto existente, guardar o barcode e resolver o mesmo bipe. Variante criada não inventa comprado nem movimenta estoque. Nome legado com tamanho final diferente não substitui o tamanho fiscal confirmado. Sem evidência suficiente, preservar a pendência e continuar a resolução em segundo plano, deixando Próximo explicitamente identificado como leitura pendente; nunca anunciar sucesso. Código efetivamente lido continua obrigatório para avançar assim; preservar captura, loja, rodada e quantidade, sem duplicação. Esta ordem atualiza a regra14 apenas quanto ao avanço consciente com código lido e resolução pendente. Fechamento da rodada continua bloqueado até resolver todas as pendências.


## 16. Confirmar código repetido antes de contar — 15/09/2026
Ordem do Douglas: se o código já tiver bipe não excluído na mesma loja e rodada, mostrar alerta com produto/referência, tamanho e quantidade anterior. Exigir confirmação explícita de que é outro par físico antes de acrescentar nova contagem. Cancelar não conta novamente. Reenvio da mesma captura mantém idempotência e não pede confirmação para duplicar o mesmo registro. Código repetido não prova erro: não excluir registros antigos automaticamente.

## Conferência parcial por variante — 16/09/2026
Pedido Douglas: disponibilizar a contagem parcial nas categorias e na consulta de estoque do vendedor. Novo read model stockVerification calcula por rodada ativa + loja + ProductSize, preservando produto/cor e sem somar StoreStock ao contado. Verde + ✓ informa quantidade validada disponível; amarelo indica pendência/reconciliação; cinza não conferido não significa falta. Evidência expansível contém barcode, operador, data e rodada. Filtros nas duas telas; totalizador de categorias em Conferidos usa apenas saldo validado. Saídas registradas (venda/transferência) posteriores à contagem reduzem saldo derivado uma vez, sem escrever estoque; entradas/ajustes, saldo negativo e movimentos intercalados exigem reconferência. Adidas com confirmação de tamanho anterior ao bipe não recebe verde. Fotos danificadas sem variante não são promovidas a saldo validado. Sem mudança fiscal, comprado ou encerramento da rodada. Estado de publicação e evidências no BIPAGEM.md da tarefa.
